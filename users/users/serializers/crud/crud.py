...
class UserCrudSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'user_request_id', 'analysis_list', 'packet_list'
        ]
    ...
    def _save_user(self, instance, validated_data):
        ...
        # Job "patient" is retrieved
        job = Job.objects.filter(name="patient").first()

        # Creating a new user instance if it's not updating an existing one
        if instance is None:
            ...
            user_request = None
            # Create a UserJob instance for the "patient" job
        ...
        # If there are associated analyses (packet_list or analysis_list)
        if user_request:
            # Remove previous user analysis records linked to the user and request
            UserAnalysis.objects.filter(user=instance, request=user_request).delete()

            # Create new user analysis records based on analysis_list and packet_list
            analysis_list = validated_data.pop('analysis_list', [])
            packet_list = validated_data.pop('packet_list', [])

            # Create multiple UserAnalysis records for analysis_list
            user_analysis_instances = [
                UserAnalysis.objects.create(user=instance, analysis_id=analysis, by_packet=False)
                for analysis in analysis_list
            ]
            # Create multiple UserAnalysis records for packet_list
            user_analysis_instances += [
